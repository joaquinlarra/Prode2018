		...
		<script>
			...
			if($phase == "qualys")
			{
				if(is_array($zones))
				{
					...
					{?>
						$('#<?= str_replace(" ","_",$zone)?>_select_1').ddslick({
							width:'100%',
							selectText: "<?= lang('select-country')?>",
							imagePosition:"left",
							onSelected: function(selectedData){
								$('#<?= str_replace(" ","_",$zone)?>_team_1_id').val(selectedData.selectedData.value);
							}   
						});
						$('#<?= str_replace(" ","_",$zone)?>_select_2').ddslick({
							width:'100%',
							selectText: "<?= lang('select-country')?>",
							imagePosition:"left",
							onSelected: function(selectedData){
								$('#<?= str_replace(" ","_",$zone)?>_team_2_id').val(selectedData.selectedData.value);
							}   
						});
				<?
					}
			...
			?>
				...
				$("#submit-btn").click(function(){
					...
					var missing_matches = "";
					var missing_zone = "";
					var total_missing_matches = 0;
					var extra_missing_matches = 0;
					var match_id = 0
					var prev_match_id = 0;
					$(".input-goals").each(function(index,value){
						if($(this).val() == "")
						{
							match_id = $(this).attr("match_id");
							match_zone = $("#matchrow-"+match_id).attr("zone");
							if((match_id != prev_match_id)&&parseInt(match_id ))
							{
								total_missing_matches += 1;
	
								if(total_missing_matches <= 5)
								{
									if(match_zone != missing_zone)
									{
										missing_matches += "<h2>"+match_zone+"</h2>";	
										missing_zone = match_zone;
									}
									missing_matches += $("#matchrow-"+match_id).attr("matchname")+"<br>";							
								}
								//$("#matchrow-"+match_id).addClass("danger");	
					...
					if(total_missing_matches > 5)
					{
						extra_missing_matches = total_missing_matches - 5;
						missing_matches += "<b>... <?= lang("y")?> "+extra_missing_matches+" <?= lang("partido")?>"+(extra_missing_matches > 1 ? "s" : "")+" <?= lang("más")?>.";
					}
					if(total_missing_matches == 0 || (missing_matches == "undefined"))
					{
						$("#prognostics-form").submit();
						return;
					}
					
					if(missing_matches != "undefined")
					{
						$("#missing-matches").html(missing_matches);
						$("#missing-matches-modal").modal();
					}
					*/
